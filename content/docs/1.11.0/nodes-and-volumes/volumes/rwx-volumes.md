...
- **Standard (Non-Migratable) RWX Volumes**
  ...
  Standard RWX volumes provide traditional shared filesystem access across multiple nodes. They use dedicated NFSv4.1 servers running in `share-manager-<volume-name>` Pods within the longhorn-system namespace. Each RWX volume is paired with a corresponding Service that exposes the NFS endpoint to clients.

  These volumes are ideal for workloads that need concurrent file access but do not require live migration.

  Characteristics:
  - Not capable of live migration
...
- **Migratable RWX Volumes**
  ...
  Migratable RWX volumes are designed specifically for virtualized workloads such as KubeVirt VMs that require live migration while maintaining ongoing I/O operations. These volumes allow seamless movement between nodes during maintenance, failover, or rebalancing events without disrupting service availability.

  Characteristics:
  - Purpose-built for live migration scenarios
  - Must use `volumeMode: Block` (`Filesystem` mode is not supported)
  - Require ReadWriteMany access mode and `volume.spec.migratable: true`
  - Not designed for general shared filesystem use cases

> **Note**: You can distinguish between migratable and non-migratable RWX volumes by checking the `volume.spec.migratable` field in the volume specification.

## Requirements for Standard (Non-Migratable) RWX Volumes
...
## Creation and Usage of Standard (Non-Migratable) RWX Volumes
...
## Configuring Volume Locality for Standard (Non-Migratable) RWX Volumes
...
## Configuring Volume Mount Options for Standard (Non-Migratable) RWX Volumes
...
## Failure Handling for Standard (Non-Migratable) RWX Volumes
